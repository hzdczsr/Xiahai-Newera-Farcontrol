...
(function() {
  ...
  M.on('model:changed', () => {
    ...
    if (chatRendered) updateWebSearchToggle();
  });
  ...
  function renderChatPanel() {
    ...
    container.innerHTML = `
      ...
      <div class="chat-subtoolbar" id="chatSubToolbar">
        ...
        <button class="stream-toggle ${M.state.webSearchEnabled ? 'active' : ''}" id="webSearchToggle" title="联网搜索">
          ${Icons.iconSearch} 联网
        </button>
      </div>
  ...
  function bindChatEvents() {
    ...
    document.getElementById('webSearchToggle')?.addEventListener('click', () => {
      ...
      btn.classList.toggle('active', !M.state.webSearchEnabled);
      M.setWebSearch(!M.state.webSearchEnabled);
    });
    ...
    document.getElementById('chatMessages')?.addEventListener('contextmenu', (e) => {
      ...
      const idx = parseInt(msgEl.id.replace('msg-', ''));
      if (!isNaN(idx)) showMessageContextMenu(e, idx);
    });
  ...
  function updateWebSearchToggle() {
    const btn = document.getElementById('webSearchToggle');
    if (!btn) return;
    // 联网搜索现已对所有模型开放（应用侧自建 DuckDuckGo 检索）
    btn.disabled = false;
    btn.style.opacity = '';
    btn.style.cursor = '';
    btn.title = '联网搜索';
  ...
  function regenerateFrom(idx) {
    ...
    M.sendMessage(userText, []);
  }

  function showMessageContextMenu(e, idx) {
    const msg = M.state.messages[idx];
    if (!msg) return;
    e.preventDefault();
    const items = [
      { label: '复制文本', icon: Icons.iconCopy, shortcut: 'Ctrl+C', action: () => copyMessageText(idx) },
      { separator: true }
    ];
    if (msg.role === 'assistant') {
      items.push({ label: '重新生成', icon: Icons.iconPlay, action: () => regenerateFrom(idx) });
    }
    items.push({ label: '删除消息', icon: Icons.iconTrash, danger: true, action: () => deleteMessage(idx) });
    M.showContextMenu({ x: e.clientX, y: e.clientY, items });
  }
